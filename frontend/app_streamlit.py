...
# Default fallback — replaced at runtime by API
...
# ─────────────────────────────────────────────────────────────────
    ...
    if not data:
        ...
        # ── 2-column layout: pie chart + category breakdown ──
                ...
                for cat, amt in sorted_cats:
                    ...
                    if st.button(
                        ...
                        if is_expanded:
                            ...
                            st.session_state.expanded_cat = f"{selected_month}_{cat}"
                        st.rerun()
                            ...
                            if txs:
                                for t in txs:
                                    ...
                                    st.markdown(
                                        ...
                                        f"{icon} <b>{t['date']}</b> · {t['description'][:30]} · "
                                        f"<span style='color:#ef4444'>-CAD$ {abs(t['amount']):.2f}</span>"
                                        f"</div>",
